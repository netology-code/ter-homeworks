...
Добавляем в `vms_platform.tf` блок с map переменной:
```
variable "vms_resources" {
  description = "Параметры ресурсов для каждой ВМ"
  type = map(object({
    cores         = number
    memory        = number
...
```
Туда же добавляем блок метадаты
```
variable "vms_metadata" {
  description = "Общая метадата для всех ВМ"
  type = map(string)
  default = {
    serial-port-enable = "1"
    ssh-keys           = "ubuntu:${var.vms_ssh_public_root_key}"
...
```
После этого в variables можно закоммеентировать эти переменные - они больше не нужны:
```
variable "vm_web_cores"
variable "vm_web_memory"
variable "vm_web_core_fraction"
